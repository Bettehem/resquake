...
object ReSquakePlayer {
  ...
  private fun PlayerEntity.travelQuake(sidemove: Double, forwardmove: Double): Boolean {
    ...
    val playerPos: BlockPos = this.blockPos
  ...
  fun PlayerEntity.surfOnStairs(world: World) {
      ...
      if (blockBelow.block is StairsBlock) {
          ...
          val grip = 0.6406 // How hard you get pulled into the stair below you. Higher value = more grip
          val stairAngle = 0.018 // Angle of the ramp. Higher value = steeper angle
          val stairPush = 2.057 // How hard you get pushed down the stairs. Higher value = stronger push
          // Calculate the normal of the stair
          val stairNormal = when (facing) {
              Direction.NORTH -> Vec3d(0.0, -grip, stairAngle)
              Direction.SOUTH -> Vec3d(0.0, -grip, -stairAngle)
              Direction.WEST  -> Vec3d(stairAngle, -grip, 0.0)
              Direction.EAST  -> Vec3d(-stairAngle, -grip, 0.0)
          ...
          val newVelocity = this.velocity.subtract(stairNormal.multiply(dot * stairPush)) // Adjust the multiplier for surf behavior
